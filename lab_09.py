...
profile = json.loads(json_dict) #takes JSON, turns it into str/list

# Convert <hobby> into a JSON named <json_list>
json_list = json.dumps(hobby) #takes object, turns into JSON
...
with open('john.json', 'w') as file_obj:
	file_obj.writelines(json_dict2) #not using loads/dumps b/c they read/write to a string, and we want to write to a file
...
with open(csv_name) as file_object:
	csv_content = csv.reader(file_object, delimiter = ',')
	row = 0
	for i in csv_content:
		if row != 0:
			animals.append(tuple(i)) #OR animals.append((i[0], i[1])
		row += 1 #do it first time, then keep adding through each loop
