...
def get_films(url, params=None):
    """
    returns a list of films from a URL

    parameters:
        url (str): where we get the films from
        params (dict): optional dictionary containing parameters that get added to URL

    returns:
        list: a list of films retrieved from the given <url> and <params>
...
def filter_properties(film):
    """
    extracts and returns properties from a given film dictionary

    parameters:
        film(dict): a dictionary containing the properties of the film

    returns:
        dict: a dictionary containing corresponding key-value pairs associated with <FILM_PROPERTIES>
    """
    record = {}
    # START: WRITE CODE BELOW
    # Extracts properties listed in <FILM_PROPERTIES> of <film> and stores them into <record>
    for key, value in film.items(): #if only said 'film,' will only return film keys...film.items() returns key-value pairs
        if key in FILM_PROPERTIES:
            record[key] = value #adds new key-value pair if doesn't exist in dictionary

    # END: WRITE CODE ABOVE
    return record
    #print(filter_properties(requests.get('https://swapi.co/api/films/1').json()))
...
def main():
    ...
    for film in films:
        film_record = filter_properties(film)
        new_film_records.append(film_record)
